...
class optStruct:
    def __init__(self,dataMatIn, classLabels, C, toler, kTup):  # 存储各类参数
        ...
        self.b = 0 #初始设为0
        self.eCache = np.zeros((self.m,2)) #缓存
        self.K = np.zeros((self.m,self.m)) #核函数的计算结果
...
def smoP(dataMatIn, classLabels, C, toler, maxIter,kTup=('lin', 0)): #输入参数：数据特征，数据类别，参数C，阀值toler，最大迭代次数，核函数（默认线性核）
    oS = optStruct(dataMatIn,classLabels.transpose(),C,toler, kTup)
    iter = 0
    entireSet = True
    alphaPairsChanged = 0
...
def testRbf(train_x, train_y, test_x, test_y):
    dataArr,labelArr = train_x, train_y #读取训练数据
    ...
    dataArr_test,labelArr_test = test_x, test_y #读取测试数据
    errorCount_test = 0
    datMat_test=test_x
    labelMat = test_y
...
def main():
    ...
    testRbf(train_x, train_y, test_x, test_y)
